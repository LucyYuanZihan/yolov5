...
class IouLoss(nn.Module):
    ...
    def forward(self, pred, target, ret_iou=False, **kwargs):
        ...
        if self.training:
            mean_iou = self['iou'].detach().mean()
            self.iou_mean.mul_(1 - self.momentum)
            # 把参与运算的张量搬到 iou_mean 的设备，避免 CPU/CUDA 混用
            self.iou_mean.add_(self.momentum * mean_iou.to(self.iou_mean.device))
    ...
    def _scaled_loss(self, loss, iou=None):
        if isinstance(self.monotonous, bool):
            iou_d = self['iou'].detach() if iou is None else iou
            beta = iou_d.to(self.iou_mean.device) / self.iou_mean
